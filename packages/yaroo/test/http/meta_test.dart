    ...
    group('Param', () {
      final pharaoh = Pharaoh()
        ..onError((error, req, res) {
          if (error is RequestValidationError) {
            return res.json(error.errorBody, statusCode: 422);
          }
      ...
      test('should use name set in meta', () async {
        pharaoh.get('/<userId>/hello', (req, res) {
          ...
          final result =
              actualParam.process(req, ControllerMethodParam('user', String));
      ...
      test(
        ...
        pharaoh.get('/boys/<user>', (req, res) {
          final result =
              param.process(req, ControllerMethodParam('user', String));
          return res.ok(result);
        });
        await (await request(pharaoh))
            .get('/boys/499')
      ...
      test('when param value not valid', () async {
        pharaoh.get('/test/<userId>', (req, res) {
          final result =
              Param().process(req, ControllerMethodParam('userId', int));
    ...
    group('Query', () {
      ...
      test(
        ...
        pharaoh.get('/bar', (req, res) {
          final result =
              query.process(req, ControllerMethodParam('userId', String));
          return res.ok(result);
        });
        await (await request(pharaoh))
    ...
    group('Header', () {
      ...
      test('should use name set in meta', () async {
        pharaoh.get('/foo', (req, res) {
          ...
          final result =
              actualParam.process(req, ControllerMethodParam('token', String));
          return res.json(result);
        });
        await (await request(pharaoh))
            .get('/foo',
                headers: {HttpHeaders.authorizationHeader: 'foo token'})
            .expectStatus(200)
