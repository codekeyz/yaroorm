...
import 'package:yaroorm/yaroorm.dart' as orm;
...
String getFieldDbName(FieldElement element, {DartObject? meta}) {
  ...
  meta ??= typeChecker(orm.TableColumn).firstAnnotationOf(element, throwOnUnresolved: false);
  if (meta != null) {
...
String getTableName(ClassElement element) {
  final meta = typeChecker(orm.Table).firstAnnotationOf(element, throwOnUnresolved: false);
...
Future<ResolvedProject> resolveMigrationAndEntitiesInDir(Directory workingDir) async {
  ...
  await for (final (library, _, _) in _libraries(collection)) {
    ...
    final config = library.element.topLevelElements
        .firstWhereOrNull((element) => typeChecker(orm.UseORMConfig).hasAnnotationOfExact(element));
    if (config != null) {
...
final class ParsedEntityClass {
  ...
  List<FieldElement> get hasManyGetters =>
      getters.where((getter) => typeChecker(orm.HasMany).isExactlyType(getter.type)).toList();

  List<FieldElement> get belongsToGetters =>
      getters.where((getter) => typeChecker(orm.BelongsTo).isExactlyType(getter.type)).toList();

  List<FieldElement> get hasOneGetters =>
      getters.where((getter) => typeChecker(orm.HasOne).isExactlyType(getter.type)).toList();
