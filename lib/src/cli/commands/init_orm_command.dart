...
final databaseInitFile =
    File('${path.join(databaseDir.path, 'database')}.dart');

final yaroormDirectory =
    path.join(Directory.current.path, '.dart_tool', 'yaroorm');
final kernelFile = File(path.join(yaroormDirectory, 'migrator.dill'));
final migratorCheckSumFile =
    File(path.join(yaroormDirectory, '.migrator_checksum'));
...
Future<void> initOrmInProject(
  ...
  final entityNames = entities
      .map((e) => e.elements.map((e) => e.name))
      .fold(<String>{}, (preV, e) => preV..addAll(e));
  ...
  final migrationFileNameDateMap = migrations
      ...
      .fold(
          <String, DateTime>{},
          (preV, filename) =>
              preV..[filename] = parseMigrationFileDate(filename));
...
''';
  ...
  final library = Library((p0) => p0
    ...
    ..directives.addAll([
      ...
      ...sortedMigrationsList.map((e) => Directive.import(
            'migrations/${path.basename(e.element.path)}',
            as: '_m${e.index}',
          ))
    ])
